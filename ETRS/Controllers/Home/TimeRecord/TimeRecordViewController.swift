...
class TimeRecordViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        query?.findObjectsInBackgroundWithBlock({ (objects, error) -> Void in
            if error == nil {
                if let objects = objects as? [PFObject], firstUser = objects.first {
                    self.users = objects as! [User]
                    print(self.users)
                    print(self.users[0].Group)
                }
            }
        })*/
    }
    ...
    func checkIn() {
        ...
        let integrityData = "\(self.beacon.UUID)_\(self.beacon.Major)_\(self.beacon.Minor)_\(self.currentUser!.username)_\(self.currentUser!.Group)_\(daymonthyear)_\(hourminute)_In"
    ...
    func checkOut() {
        ...
        print(day, month, year, currentUser!.username!)
        ...
        query?.getFirstObjectInBackgroundWithBlock({ (object, error) -> Void in
            if error == nil {
                ...
                let integrityData = "\(self.beacon.UUID)_\(self.beacon.Major)_\(self.beacon.Minor)_\(self.currentUser?.username)_\(self.currentUser?.Group)_\(objectTR.Day)-\(objectTR.Month)-\(objectTR.Year)_\(objectTR.StartTimeRecord)_In_\(hourminute)_Out"
